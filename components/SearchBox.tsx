...
export default function SearchBox({ basePath = '', isCR = false, placeholder = 'Buscar productos...', variant = 'dark' }: Props) {
  ...
  return (
    <div ref={wrapRef} className="relative w-full">
      ...
      {open && (
        <div className="absolute left-0 right-0 top-full mt-2 bg-white rounded-2xl shadow-2xl border border-gray-100 z-[70] p-4 max-h-[70vh] overflow-y-auto">
          {recent.length > 0 && (
            <div className="mb-4">
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {(isCR ? CR_POPULAR : CR_POPULAR).map((p) => (
          ...
          {isCR && (
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {CR_CATS.map((c) => (
                  <Link key={c.href} href={c.href} onClick={() => setOpen(false)} className="px-3 py-2 rounded-xl bg-gray-50 border border-gray-100 text-sm font-medium text-navy hover:border-brand-orange transition-colors">
                    {c.label}
                  </Link>
                ))}
              </div>
